...
def dump_predictions(pred, lbl_set, img, prefix):
    ...
    # if '/vip/' in prefix:
    #     min_val = np.min(pred_lbl)
    #     max_val = np.max(pred_lbl)
    #     if max_val == min_val:
    #         pred_lbl = np.zeros_like(pred_lbl)
    #     else:
    #         mask = pred_lbl == 0
    #         pred_lbl = (pred_lbl - min_val) / (max_val - min_val)
    #         colormap = cm.get_cmap('plasma')
    #         pred_lbl_colored = colormap(pred_lbl[:, :, 0])[:, :, :3] * 128
    #         pred_lbl_colored[mask] = 0
    #         pred_lbl = pred_lbl_colored
