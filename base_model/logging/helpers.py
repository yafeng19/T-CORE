...
class MetricLogger(object):
    def __init__(self, delimiter="\t", output_file=None, tb_file=None):
        self.meters = defaultdict(SmoothedValue)
        self.delimiter = delimiter
        self.output_file = output_file
        if tb_file is not None and distributed.is_main_process():
            self.writer = SummaryWriter(tb_file)
        else:
            self.writer = None

    def update_tensorboard(self, tags, values, prefix):
        ...
        for tag, value in zip(tags, values):
            self.writer.add_scalar(prefix + '/' + tag, value, global_step=self.iteration)
